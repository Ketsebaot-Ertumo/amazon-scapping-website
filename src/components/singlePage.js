...
const ProductTable1 = () => {
    ...
    const fetchProducts = async () => {
        try {
            ...
            const response = await axios.get(
                `${url}/product/?key=${apiKey}&domain=1&asin=${asin}&fields=${fields}`
            );
            const foundProducts = response.data.products[0];
            const offers = foundProducts.offers; // Assuming offers is present
            ...
            //     purchasableUnits
            // };
            setProducts(foundProducts.imagesCSV);
        } catch (err) {
    ...
    const columns = [
        ...
        //     width: 300,
        //     renderCell: (params) => (
        //         console.log('images',params.row.imagesCSV),
        //         params.row.imagesCSV ? params.row.imagesCSV.split(',').map((image, index) => (
        //             <img 
        //                 key={index} 
        //                 src={image} 
        //                 alt={`Image-${index + 1}`} 
